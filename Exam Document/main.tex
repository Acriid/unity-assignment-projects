...
through aesthetic and themes of the game not really the mechanics of the game (reference). The survival horror genre is
...
feel powerless, by giving them multiple means to fight or run away , but those means are not reliable and can fail the player.

\subsection{Alien: Isolation}
Alien: Isolation(reference) is a survival horror game set in a space station. The general premise of the game is to survive
...
the player to see if the Alien is around then ,but the Alien can hear the motion tracker. The player is also given
...
Dead By Daylight is a action survival horror game set in an eldritch realm. There are two parts of the gameplay of 
...
Most of the actions that the player needs to do to progress is communicated from the dialog system where if the
...
different mechanic first. As seen in the (ref figures) the player chooses which mechanic they want to learn
and get moved to an area that teaches that mechanic (ref figure) , instead of moving linearly through the
mechanics (ref figure).
...
the rest of the map but contamination(word) was added to indicate the player is not supposed to be there the difference
from the original prototype can be seen in (ref figures).
...
causing the player to get lost in the map a bit too often size change can be seen in (ref fig1) and (ref fig 2).
...
a sense of direction the minimap is in the bottom right corner of the screen and can be seen in (ref fig),
...
% https://en.wikipedia.org/wiki/Horror_game
% https://en.wikipedia.org/wiki/Horror_fiction
% https://www.gamedeveloper.com/design/what-makes-the-survival-of-survival-horror-the-difference-between-scares-and-shooting
% https://www.gamedeveloper.com/design/why-horror-is-the-hardest-game-genre
